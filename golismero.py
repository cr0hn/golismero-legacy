...
##    gr_main.add_argument("--forward-io", metavar="ADDRESS:PORT", help="forward all input and output to the given TCP address and port")
    ...
    gr_audit.add_argument("--dump-db", metavar="FILENAME", default=None, help="dump the audit database and quit")
    ...
    if P.dump_db:
        ...
        try:
            with PluginTester(autoinit=False, autodelete=False) as t:
                ...
                t.init_environment()
                if P.verbose != 0:
                    print "Dumping to file: %s" % P.dump_db
                t.audit.database.dump(P.dump_db)
            exit(0)
        except Exception, e:
            raise # XXX DEBUG
            print "Error! " + str(e)
