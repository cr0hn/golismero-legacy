...
"""
This package contains the classes that represent the differents types of DNS types and responses.
"""
...
class DnsSEC:
	...
	ALGORITHM_BY_NUM = dict([(y, x) for x, y in DnsSEC.ALGORITHM_BY_TEXT.iteritems()])
...
class DnsRegister(object):
	...
	def __init__(self, **kwargs):
		...
		:param domain_servers: list with the domain servers associated to this record.
		:type domain_servers: list(str)
		"""
		self._type               = None
		...
		try:
			self._domain_servers = kwargs['domain_servers']
		except KeyError:
			pass
		...
		if isinstance(self._domain_servers, list):
			for d in self._domain_servers:
				if not isinstance(d, basestring):
					raise TypeError("Expected str, got %s" % type(d))
		else:
			raise TypeError("Expected list, got %s" % type(self._domain_servers))
	...
	def domain_servers(self):
		"""
		:return: list with the name of domain servers.
		:rtype: list(str)
		"""
...
class DnsRegisterAAAA(DnsRegister):
	...
	def __init__(self, name, address, **kwargs):
		...
		# Set type of register and the other options
		super(DnsRegisterA, self).__init__(type="AAAA", **kwargs)
...
class DnsRegisterCNAME(DnsRegister):
	...
	def __init__(self, name, target, **kwargs):
		...
		:param name: name of alias CNAME.
		:type name: str.
...
if __name__ == "__main__":
	p = DnsSEC.algorithm_to_text(self, alg)