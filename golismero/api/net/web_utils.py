...
def download(url, callback = None, timeout = None, allow_redirects = True):
    ...
    req = HTTP_Request(      url = url.url,
                          method = url.method,
                       post_data = url.post_params,
                         referer = url.referer )
    LocalDataCache.on_autogeneration(req)

    # Prepare the callback.
    if callback is None:
        temp_callback = None
    else:
        def temp_callback(req, url, status_code, content_length, content_type):
    ...
    if resp:
        ...
        LocalDataCache.on_autogeneration(resp)

        # Associate the URL to the request and the response.
        req.add_resource(url)
        resp.add_resource(url)

        # Extract the data from the response.
        # TODO: maybe move this bit to its own function?
        ...
        if content_type == "text/html":
            ...
            data = Binary(resp.data)

        # Associate the data to the response and the URL.
        data.add_information(resp)
